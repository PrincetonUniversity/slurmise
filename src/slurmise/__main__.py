...
from slurmise import parse_args,slurm,job_database,job_data

@click.group()
@click.option('--database','-d', type=click.Path(exists=False), default='.slurmise.h5', help="Path to the hdf5 database file")
@click.pass_context
...
@click.option("--numerical", type=str, help="Numerical run parameters in JSON format without outer {}, such as 'n:3,q:17.4'")
@click.option("--categorical", type=str, help="Categorical run parameters in JSON format without outer {}")
